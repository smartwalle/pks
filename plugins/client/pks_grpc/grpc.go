...
func init() {
	...
	cmd.DefaultClients[kClientName] = NewClient
}
...
func (g *grpcClient) stream(ctx context.Context, node *registry.Node, req client.Request, opts client.CallOptions) (client.Stream, error) {
	...
	var cc *poolConn
	for {
		cc, err = g.streamPool.getConn(address, grpc.WithDefaultCallOptions(grpc.CallCustomCodec(cf)), grpc.WithTimeout(opts.DialTimeout), g.secure())
		if err != nil {
